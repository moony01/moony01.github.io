...
GeekNews에서 최근 며칠간 `agent-skills`, Managed Agents, HyperAgents 같은 글이 같이 회자된 것도 같은 맥락이다. 에이전트가 더 많은 권한과 더 긴 실행 시간을 갖게 될수록, "정답을 잘 찾는 능력"과 "평가 환경을 잘 우회하는 능력"이 동시에 커진다. 며칠 전 정리했던 [MCP와 Skills 연결 방식의 보안 문제](/others/2026/04/10/mcp-vs-skills-shift.html)도 결국 같은 질문으로 이어진다. 자동화 자산이 커질수록 검증 입력도 공격 표면이 된다.
